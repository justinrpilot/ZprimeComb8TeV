void draw(TString fname = "qcdClosure_lowDY.root", int lohiDY = 0)
{
 ...
 //TGraphAsymmErrors* mc_eff = (TGraphAsymmErrors*) file->Get("qcd3predH");
  ...
  // rebin!
  double binning[] = {500, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000, 2100, 2200, 2300, 2400, 2500, 2600, 2800, 3000, 3500, 4000};

  data_eff = (TH1F*) data_eff->Rebin(24, "sel", binning);
  mc_eff = (TH1F*) mc_eff->Rebin(24, "pred", binning);
  ...
  TLine* l = new TLine(800, 1, 4000, 1);
  ...
  ratio->GetXaxis()->SetRangeUser(800, 4000);
