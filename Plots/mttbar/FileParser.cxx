...
//     TH1* rebinned = hist->Rebin(2);
//     rebinned->SetTitle("missing E_{T} [GeV]");
//     return rebinned;

//   } else if (name.Contains("pT_1") && title.Contains("muon")) {
    
//     TH1* rebinned = hist->Rebin(2);
//     rebinned->SetTitle("p_{T} leading muon [GeV]");
//     return rebinned;

//   }
//   return NULL;

  // use this to get all numbers for expected and observed events
  //   Double_t binsx[] = {0, 5000};
  //   name.Append("_rebin_lx");
  //   TH1* rebinned = hist->Rebin(1, name, binsx);
  ...
  if (name.Contains("TopTag_1_pt")){
    //for (int i=1; i<hist->GetNbinsX(); ++i){
    //  cout << hist->GetXaxis()->GetBinLowEdge(i) << ", ";
    //}
    //cout << endl;
    Double_t binsx[] = {0, 200, 400, 440, 480, 520, 560, 600, 640, 680, 720, 780, 800, 840, 880, 920, 960, 1000, 1040,  1080, 1120, 1160, 1200, 2000};
    name.Append("_rebin_ly");
    TH1* rebinned = hist->Rebin(23, name, binsx);
    rebinned->SetTitle("p_{t} [GeV]");
    //rebinned->GetXaxis()->SetRangeUser(200,1200);
    return rebinned;
  }
  ...
  if (name == "M_ttbar_rec" || name == "M_ttbar_rec_ly") {
    TH1* rebinned = hist->Rebin(2);
    rebinned->GetXaxis()->SetRangeUser(0,3500);
    rebinned->GetXaxis()->SetLimits(0,3500);
    rebinned->SetTitle("M_{t#bart} [GeV]");
    return rebinned;
  }
