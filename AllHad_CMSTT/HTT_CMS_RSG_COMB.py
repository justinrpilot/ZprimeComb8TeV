def build_model(rootfile):
    ...
    model.set_signal_processes('RS*')
    for p in model.processes:
       if p == 'qcd': continue
       model.add_lognormal_uncertainty('lumi', 0.026, p)
       #model.add_lognormal_uncertainty('btag', 0.05, p)
...
model = build_model('RSG_templates_May20.root')
HTTmodel = build_HTT_model('htt_rsg-1.root')
...
plot_exp.write_txt('exp_limit_RSG_comb.txt')
plot_obs.write_txt('obs_limit_RSG_comb.txt')
